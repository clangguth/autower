/*
ACKNOWLEDGEMENT: This file is originally part of the FSUIPC SDK.
and should be treated according to the corresponding license.
All credits go to the original author(s).
I have only made some minor changes to it, and combined
both IPC access methods in a single file.

This file is explicitly NOT covered by the GPL, and
...
/* Note: if you build with GCC, you'll have to define
   _ATT_SYNTAX for the compiler.

  Also, make sure that you define BUILD_DLL when building a DLL.
*/

#ifdef BUILD_DLL // build internal module access
